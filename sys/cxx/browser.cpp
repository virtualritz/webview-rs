...
IBrowser::IBrowser(PageOptions settings, PageObserver observer, void *ctx)
    : _settings(settings), _observer(observer), _ctx(ctx),
      IRender(settings, observer, ctx), IDisplay(settings, observer, ctx) {}

CefRefPtr<CefDragHandler> IBrowser::GetDragHandler() { return this; }
...
void IBrowser::SetDevToolsOpenState(bool is_open) {
  ...
  if (is_open) {
    _browser.value()->GetHost()->ShowDevTools(CefWindowInfo(), nullptr,
...
const void *IBrowser::GetHWND() {
  return _browser.has_value()
             ? (void *)_browser.value()->GetHost()->GetWindowHandle()
             : nullptr;
}
